...
const REG_STATUSES = ["active", "expired", "suspended"];
...
export default function Registrations() {
  ...
  const fetchRegistrations = useCallback(async () => {
    ...
    try {
      ...
      if (filterStatus && !search.trim()) {
        list = list.filter((r) => r.registration_status === filterStatus);
      }
  ...
  const clearFilters = () => { setSearch(""); setFilterStatus(""); setSortBy(""); setOrder("asc"); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "add") {
        ...
        res = await fetch(`${API}/${selected.registration_number}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      }
      const data = await res.json();
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <div className={styles.headerRight}>
          <input className={styles.search} placeholder="Search by reg #, plate…" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      <div className={`${styles.body} ${panelOpen ? styles.bodyWithPanel : ""}`}>
        <div className={styles.tableWrap}>
          ...
          {loading ? <div className={styles.empty}>Loading…</div>
            : registrations.length === 0 ? <div className={styles.empty}>No registrations found.</div>
            : (
                ...
                <tbody>
                  {registrations.map((r) => (
                    <tr key={r.registration_number}
                      ...
                      <td><span className={`${styles.statusBadge} ${statusClass(r.registration_status)}`}>{r.registration_status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {panelOpen && (
          <div className={styles.panel}>
            ...
            <div className={styles.panelBody}>
              {mode === "view" && selected && (
                <>
                  ...
                  <DetailRow label="Status">
                    <span className={`${styles.statusBadge} ${statusClass(selected.registration_status)}`}>{selected.registration_status}</span>
                  </DetailRow>
              ...
              {(mode === "add" || mode === "edit") && (
                <>
                  <FormField label="Registration #" name="registration_number" value={form.registration_number} onChange={handleField} disabled={mode === "edit"} />
                  <FormField label="Plate #" name="plate_number" value={form.plate_number} onChange={handleField} disabled={mode === "edit"} />
                  <FormField label="Registration Date" name="registration_date" type="date" value={form.registration_date} onChange={handleField} />
                  <FormField label="Expiration Date" name="expiration_date" type="date" value={form.expiration_date} onChange={handleField} />
                  <FormSelect label="Status" name="registration_status" value={form.registration_status} onChange={handleField} options={REG_STATUSES} />
      ...
      {deleteConfirm && (
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            ...
            <p className={styles.modalBody}>Permanently delete registration <strong>{deleteConfirm}</strong>? This cannot be undone.</p>
